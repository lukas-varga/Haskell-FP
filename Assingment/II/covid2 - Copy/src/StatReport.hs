...
import CovidData ( field2fun, CField, CovidData(date), toDouble, DefaultToZero, makeDate)
...
computeRange get covid startString endString  = (avg, low, high, daysBetween) 
  where
    ...
    sorted = sort (toList covid)
    select = [ x | x <- sorted, date x >= startDay && date x <= endDay ] 

    low = get $ head select
    high = get $ last select
    daysBetween = fromIntegral $ abs $ diffDays startDay endDay

    selectAVG = fmap get select
    sortedAVG = sort selectAVG
    avg = sum sortedAVG / fromIntegral (length sortedAVG)

userStatInfo :: (Functor t, Foldable t) => t CovidData -> String -> String -> [UserStat]
userStatInfo covid start end = fmap cFieldUserStatInfo [minBound .. maxBound]
  where
--    decimalPlacesByCField Volume = 0                         
    decimalPlacesByCField _ = decimalPlacesFloating

    cFieldUserStatInfo cfieldUser =
      let
        get = field2fun cfieldUser
        (mnUser, mxUser, daysBetweenMinMaxUser) =
              computeMinMaxDays get covid
        decPlaces = decimalPlacesByCField cfieldUser
        meanUser = StatValue decimalPlacesFloating
                            (mean $ fmap get covid)
        minUser = StatValue decPlaces mnUser
        maxUser = StatValue decPlaces mxUser
      in UserStat {..}

instance Buildable UserStat where
  build UserStat {..} =
           ""+||cfieldUser||+": "
             +|meanUser|+" (mean), "
             +|minUser|+" (min), "
             +|maxUser|+" (max), "
             +|daysBetweenMinMaxUser|+" (days)"



userTextReport :: [UserStat] -> String
userTextReport = ascii colStats
  where
    colStats = mconcat
      [ headed "Covid Field" (show . cfieldUser)
      , headed "Mean" (pretty . meanUser)
      , headed "Min" (pretty . minUser)
      , headed "Max" (pretty . maxUser)
      , headed "Days between Min/Max" (pretty . daysBetweenMinMaxUser)
      ]
